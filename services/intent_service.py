...
class __IntentService:
    ...
    def load(self) -> None:
        ...
        """
        if self._recognizer is not None and self._recognizer.is_ready:
            return
        with self._lock:
            if self._recognizer is not None and self._recognizer.is_ready:
                return
    ...
    def _ensure_loaded(self) -> EmbeddingIntentRecognizer:
        """懒加载：首次调用推理前自动完成初始化。"""
        if self._recognizer is None or not self._recognizer.is_ready:
            self.load()
    ...
    async def recognize(
        ...
        logger.debug(f"[IntentService] 识别请求: text='{text[:50]}...' " if len(text) > 50 else f"[IntentService] 识别请求: text='{text}'")
        try:
    ...
    async def reload_intents(
        ...
        recognizer = self._ensure_loaded()
        logger.info(
            f"[IntentService] 热更新意图字典: {len(intent_dict)} 个意图"
        )
        try:
            recognizer.update_intents(intent_dict)
